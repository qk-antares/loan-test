...
class CategoryEmbedding(nn.Module):
    ...
    def forward(self, onehot_input):
        # onehot_input: (batch_size, vocab_size)
        # 将独热编码转换为索引
        indices = torch.argmax(onehot_input, dim=1)  # (batch_size,)
        embedded = self.embedding(indices)  # (batch_size, embed_dim)
...
class LoanDistributionModel:
    ...
    4. 特征降维：对高维分类特征进行神经网络降维
    """
    
    def __init__(self, processed_data_dir: str = "processed_data", use_embedding: bool = True):
        """
        初始化模型
        
        Args:
            processed_data_dir: 处理后数据目录
            use_embedding: 是否使用嵌入降维
        """
        ...
        self.encoders = {}  # 编码器
        self.embeddings = {}  # 嵌入模块
        self.scaler = StandardScaler()
        self.company_filter_rules = self._init_company_filters()
        self.use_embedding = use_embedding and TORCH_AVAILABLE
        
        # 定义需要降维的高维分类特征
        self.high_dim_features = ['city', 'province', 'idInfo.nation']
        self.embedding_dims = {'city': 32, 'province': 16, 'idInfo.nation': 12}  # 降维后的维度
    ...
    def preprocess_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 4. 特殊处理联系人关系特征 - 使用计数编码
        relationship_features = ['linkmanList.0.relationship', 'linkmanList.1.relationship']
        if all(feat in processed_df.columns for feat in relationship_features):
            print("  处理联系人关系特征 - 计数编码...")
            
            # 获取所有可能的关系类型
            all_relationships = set()
            for feat in relationship_features:
                processed_df[feat] = processed_df[feat].fillna('UNKNOWN')
                all_relationships.update(processed_df[feat].astype(str).unique())
            
            all_relationships = sorted(list(all_relationships))
            print(f"    发现关系类型: {all_relationships}")
            
            # 创建10维计数向量
            for rel in all_relationships:
                col_name = f'relationship_count_{rel}'
                processed_df[col_name] = 0
                
                # 统计每种关系出现的次数
                for feat in relationship_features:
                    processed_df.loc[processed_df[feat] == rel, col_name] += 1
            
            # 移除原始关系特征
            for feat in relationship_features:
                if feat in processed_df.columns:
                    processed_df = processed_df.drop(feat, axis=1)
        
        # 5. 处理分类特征 - 分为高维和低维分别处理
        categorical_features = [
            'bankCardInfo.bankCode', 'city', 'companyInfo.industry', 
            'companyInfo.occupation', 'customerSource', 'idInfo.gender',
            'idInfo.nation', 'jobFunctions', 'maritalStatus', 'province',
        ...
        for feature in categorical_features:
            if feature in processed_df.columns:
                ...
                # 获取该特征的所有唯一值
                unique_values = processed_df[feature].astype(str).unique()
                n_categories = len(unique_values)
                print(f"    {feature}: {n_categories} 个类别")
                
                # 对于高维特征，如果启用嵌入，则保存为整数编码
                if feature in self.high_dim_features and self.use_embedding:
                    print(f"      -> 将使用嵌入降维到 {self.embedding_dims[feature]} 维")
                    
                    if f'{feature}_encoder' not in self.encoders:
                        # 训练时：创建标签编码器用于嵌入
                        from sklearn.preprocessing import LabelEncoder
                        self.encoders[f'{feature}_encoder'] = LabelEncoder()
                        encoded_values = self.encoders[f'{feature}_encoder'].fit_transform(processed_df[feature].astype(str))
                    else:
                        # 预测时：使用已保存的编码器
                        encoded_values = self.encoders[f'{feature}_encoder'].transform(processed_df[feature].astype(str))
                    
                    processed_df[f'{feature}_encoded'] = encoded_values
                ...
                if feature not in self.encoders:
                    ...
                    # 对当前数据进行独热编码
                    feature_data = processed_df[feature].astype(str).values.reshape(-1, 1)
                    onehot_encoded = self.encoders[feature].fit_transform(feature_data)
                    
                    # 生成新的列名
                    feature_names = [f'{feature}_{cat}' for cat in self.encoders[feature].categories_[0]]
                    
                    # 添加独热编码后的列到数据框
                    onehot_df = pd.DataFrame(onehot_encoded, columns=feature_names, index=processed_df.index)
                    processed_df = pd.concat([processed_df, onehot_df], axis=1)
                else:
                    # 预测时：使用已保存的编码器
                    feature_data = processed_df[feature].astype(str).values.reshape(-1, 1)
                    onehot_encoded = self.encoders[feature].transform(feature_data)
    ...
    def _apply_embedding_reduction(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        对高维分类特征应用嵌入降维
        ...
        for feature in self.high_dim_features:
            encoded_col = f'{feature}_encoded'
            if encoded_col in processed_df.columns:
                # 获取词汇表大小
                vocab_size = len(self.encoders[f'{feature}_encoder'].classes_)
                embed_dim = self.embedding_dims[feature]
                
                print(f"    {feature}: {vocab_size} -> {embed_dim} 维")
                
                # 创建或获取嵌入模块
                if feature not in self.embeddings:
                    self.embeddings[feature] = CategoryEmbedding(vocab_size, embed_dim)
                    self.embeddings[feature].eval()  # 设置为评估模式
                
                # 转换数据
                encoded_values = processed_df[encoded_col].values
                
                # 转换为PyTorch张量
                indices_tensor = torch.LongTensor(encoded_values)
                
                # 创建独热编码张量
                onehot_tensor = torch.zeros(len(encoded_values), vocab_size)
                onehot_tensor[torch.arange(len(encoded_values)), indices_tensor] = 1
                
                # 应用嵌入
                with torch.no_grad():
                    embedded = self.embeddings[feature](onehot_tensor)
                    embedded_np = embedded.numpy()
                
                # 添加嵌入特征到数据框
                for i in range(embed_dim):
                    processed_df[f'{feature}_embed_{i}'] = embedded_np[:, i]
                
                # 移除原始编码列
                processed_df = processed_df.drop(encoded_col, axis=1)
    ...
    def prepare_training_data(self, df: pd.DataFrame) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        ...
        """
        # 应用嵌入降维
        df_with_embeddings = self._apply_embedding_reduction(df)
        
        # 分离特征和标签
        feature_columns = [col for col in df_with_embeddings.columns 
                          if col not in ['partner', 'partner_label']]
        
        X = df_with_embeddings[feature_columns].values
        ...
        for partner in self.partners:
            partner_mask = df_with_embeddings['partner'] == partner
            partner_indices = df_with_embeddings.index[partner_mask].tolist()
            Y_dict[partner] = {
                'X_indices': partner_indices,
                'labels': df_with_embeddings.loc[partner_mask, 'partner_label'].values
            }
    ...
    def _train_embeddings(self, df: pd.DataFrame):
        """
        训练嵌入模块
        ...
        Args:
            df: 包含所有数据的DataFrame
        """
        if not self.use_embedding or not TORCH_AVAILABLE:
            return
        ...
        for feature in self.high_dim_features:
            ...
            print(f"  训练 {feature} 嵌入...")
            
            # 获取数据
            encoded_values = df[encoded_col].values
            vocab_size = len(self.encoders[f'{feature}_encoder'].classes_)
            embed_dim = self.embedding_dims[feature]
            
            # 创建嵌入模块
            embedding = CategoryEmbedding(vocab_size, embed_dim)
            ...
            # 创建数据加载器
            dataset = TensorDataset(onehot_tensor, onehot_tensor)  # 输入=输出（重构）
            dataloader = DataLoader(dataset, batch_size=128, shuffle=True)
            
            # 训练
            embedding.train()
            for epoch in range(20):  # 少量epochs即可
                total_loss = 0
                for batch_onehot, target_onehot in dataloader:
                    optimizer.zero_grad()
                    
                    # 前向传播
                    embedded = embedding(batch_onehot)  # (batch, embed_dim)
                    
                    # 重构回原始维度（简单的线性层）
                    if not hasattr(embedding, 'decoder'):
                        embedding.decoder = nn.Linear(embed_dim, vocab_size)
                    
                    reconstructed = embedding.decoder(embedded)  # (batch, vocab_size)
                    
                    # 计算损失
                    loss = criterion(reconstructed, target_onehot)
                    loss.backward()
                    optimizer.step()
    ...
    def compare_imbalance_strategies(self, df: pd.DataFrame, X: np.ndarray, Y_dict: Dict[str, np.ndarray]):
        """
        比较不同的不平衡处理策略
    ...
    def save_model(self, model_path: str = "loan_distribution_model"):
        ...
        # 保存嵌入模块
        if self.use_embedding and TORCH_AVAILABLE:
            embeddings_path = os.path.join(model_path, 'embeddings')
            os.makedirs(embeddings_path, exist_ok=True)
            for feature, embedding in self.embeddings.items():
                torch.save(embedding.state_dict(), os.path.join(embeddings_path, f'{feature}.pt'))
    ...
    def load_model(self, model_path: str = "loan_distribution_model"):
        ...
        if self.use_embedding and TORCH_AVAILABLE:
            ...
            if os.path.exists(embeddings_path):
                for feature in self.high_dim_features:
                    embedding_file = os.path.join(embeddings_path, f'{feature}.pt')
                    if os.path.exists(embedding_file):
                        vocab_size = len(self.encoders[f'{feature}_encoder'].classes_)
                        embed_dim = self.embedding_dims[feature]
                        embedding = CategoryEmbedding(vocab_size, embed_dim)
                        embedding.load_state_dict(torch.load(embedding_file))
                        embedding.eval()
                        self.embeddings[feature] = embedding
...
def main():
    ...
    # 4. 比较不同的不平衡处理策略
    comparison_results = model.compare_imbalance_strategies(processed_data, X, Y_dict)
